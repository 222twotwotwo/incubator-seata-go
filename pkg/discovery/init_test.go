...
func TestInitRegistryWithErrorUnimplementedTypes(t *testing.T) {
	tests := []struct {
		name         string
		registryType string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := InitRegistryWithError(&ServiceConfig{}, &RegistryConfig{Type: tt.registryType})
			if err == nil {
				t.Fatal("expected error")
			}
			want := fmt.Sprintf("registry type %s is not implemented yet", tt.registryType)
			if err.Error() != want {
